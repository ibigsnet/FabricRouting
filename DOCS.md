...
How FabricRouting installs and manages the [FRRouting](https://frrouting.org/) suite on Unraid—package catalog download via **Download & Install packages** (openBox), daemon selection, and safe defaults for the rest of the network stack.
...
**[docs/first-time-setup.md](docs/first-time-setup.md)** — do I need it, plugin map, first package download (openBox), what each setting means, success checks, common mistakes.
...
| **Companion marker** | `companion.json` so Thunderbolt Net can detect FabricRouting |

You choose a package radio and click **Download & Install packages**. You do **not** need to manually copy packages once the flash cache is populated.
...
| Download & Install | Explicit button | openBox catalog job (not Settings Apply) |
...
| Start FRR on Apply settings | **Yes** | Bring stack up after daemon changes |
...
1. **Apps** tab → search **FRR** or **FabricRouting**.  
2. **Install** or **Update**.  
3. Hard-refresh the browser (**Ctrl+Shift+R**).  
4. Open **Settings → Network Settings → Fabric Routing** (tab with eth / Routing Table / Thunderbolt).  
5. First time: **Download & Install packages** (leave openBox open until DONE).

CA is fed from [unraid-templates](https://github.com/ibigsnet/unraid-templates) (`plugins/fabricrouting.xml`). Updates may lag a short time after GitHub.
...
1. **Download & Install packages** so the plugin can fetch a matching package set when one exists in the catalog.  
...
3. **Remove** emhttp plugin paths (`/usr/local/emhttp/plugins/FabricRouting`, case variants).  
4. **Remove** flash state under `/boot/config/plugins/FabricRouting` (config, package cache, marker)—full clean so a later reinstall starts fresh.  
...
| Step | Action |
|------|--------|
| 1 | Install FabricRouting → Download & Install packages (FRR present when catalog matches) |
