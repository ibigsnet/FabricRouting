...
| Symptom | Root cause |
|---------|------------|
| `plugin remove` said removed, but FRR packages + emhttp + flash remained | Plugin **not registered** (`/var/log/plugins/fabricrouting.plg` missing). Unraid only runs `Method=remove` when the installed plg path is valid; orphaned files need force cleanup. |
...
| Array never auto-starts after reboot | Boot stuck on FabricRouting package fetch inside `plugin install` (see boot-blocker-plugin-install-stall.md). Same root cause. |
...
1. `plugin install …/fabricrouting.plg` completes in **seconds** (plugin files + cfg only — **no** FRR package download).  
2. Symlink `/var/log/plugins/fabricrouting.plg` → `/boot/config/plugins/fabricrouting.plg` exists.  
...
1. Plugins → Remove (or `plugin remove fabricrouting.plg`) **with symlink present**.  
2. After remove:
   - No `vtysh` / no frr|libyang under `/var/log/packages`
   - No FRR processes  
   - No `/usr/local/emhttp/plugins/FabricRouting`  
   - No `/boot/config/plugins/FabricRouting`  
   - No `/etc/frr` (we wipe plugin-managed conf)  
3. `network.cfg` / br0 / Wi‑Fi / Docker / Thunderbolt Net **untouched**.

**Broken half-install:** run  
`/usr/local/emhttp/plugins/FabricRouting/scripts/frr-force-cleanup`  
...
plugin remove fabricrouting.plg
